...
import { persistLang } from '../../language-storage';
...
export class NavbarComponent implements AfterViewInit, OnDestroy {
  ...
  currentLang = this.translate.currentLang || this.translate.defaultLang || 'en';
  favoritesCount = 0;
  ...
  ngAfterViewInit(): void {
    ...
    this.sub.add(
      this.authState.currentUser$.subscribe((u) => {
        if (!u) {
          this.favoritesCount = this.guestFavs.ids().size;
          this.cdr.markForCheck();
          return;
        }
        this.favoritesApi.count().subscribe({
          next: (r) => {
            this.favoritesCount = Math.max(0, Number(r?.count ?? 0));
            this.cdr.markForCheck();
          },
          error: () => {
  ...
  displayName(user: import('firebase/auth').User | null, profile: import('../../../models/user-profile.model').UserProfile | null): string {
